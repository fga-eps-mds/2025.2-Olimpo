          ...
          <form className={`${styles.formulario} ${styles["formulario-scroll"]}`} onSubmit={handleSubmit}>
            ...
            <div className={styles["senha-requisitos"]} style={{marginBottom: 10, marginLeft: 16}}>
              {rules.map(rule => {
                ...
                return (
                  <div key={rule.label} style={{ display: 'flex', alignItems: 'center', marginBottom: 5 }}>
                    {passed ? <CiCircleCheck size={18} color="#FDC700" /> : <CiCircleRemove size={18} color="#fff" />}
                    <span style={{ marginLeft: 8, color: passed ? '#FDC700' : '#fff', fontWeight: passed ? 'bold' : 'normal', fontSize: '14px' }}>
                      {rule.label}
                    </span>
                  </div>
                );
